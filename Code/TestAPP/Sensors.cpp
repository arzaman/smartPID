...
volatile uint16_t temp_val1=22,temp_val2=102;
...
void MathSensorsValues(void)
{
  ...
  tsens = *((uint16_t*)temp_sens1);
  sensor1_fval=tsens>>4;
  sensor1_fval+=(double)(tsens & 0x0f)*0.0625;
  }
  if(_stv.HW_var.probe2_mode==SENSOR_ONEWIRE) {
  tsens = *((uint16_t*)temp_sens2);
  sensor2_fval=tsens>>4;
  sensor2_fval+=(double)(tsens & 0x0f)*0.0625;
  }
