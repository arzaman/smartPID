...
void WiFi::config(int mode, const char *ssid, const char *pwd) {
	...
	case MODE_STA:
		...
		break;
	case MODE_AP:
		configAP(WIFI_OWN_SSID, WIFI_OWN_PWD);
		break;
	case MODE_AUTO:
		if (*ssid && *pwd) {
			configSta(ssid, pwd);
...
void WiFi::configAP(const char *ssid, const char *pwd) {
	...
	if (strcmp(cur_ssid, ssid) || strcmp(cur_pwd, pwd)) {
		while (esp8266.configAP(ssid, pwd, true) == ESP8266_CMD_BUSY) {}
