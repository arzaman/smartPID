...
static 
//__attribute__((__aligned__(4))) //__attribute__((__section__(".bss_hram0")))
...
uint32_t USBDeviceClass::send(uint32_t ep, /*const*/ uint8_t/*void*/ *data, uint32_t len)
{
	uint32_t length = 0;
...
// this part
	...
	while (len != 0)
	{
		...
		len -= length;
//		const void *tl=(void const*)length;
//                data += tl;
//                uint8_t *adder;
//                adder=(uint8_t*)length;
                data += length;
//                data += (const uint8_t*)length;
	}
	return len;
}
